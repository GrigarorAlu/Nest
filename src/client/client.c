...
static void onAccountOpen(int id)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (sqlite3_step(sub) == SQLITE_ROW)
        {
            chat.lastMessageId = sqlite3_column_int(sub, 0);
            chat.lastMessageContent = strdup(sqlite3_column_text(sub, 1));
        }
        else
        {
            chat.lastMessageId = 0;
            chat.lastMessageContent = 0;
        }
        sqlite3_reset(sub);
...
static int processChatData(char *data, size_t dataLen)
{
    int chatId;
    int ownerId;
    int lastMessageId;
    ...
    size_t memberCount = (dataLen - serializeLen("iiisstt", name, lastMessageContent)) * 8;
    ...
    if (sqlite3_step(stmt) != SQLITE_CONSTRAINT)
    {
        ...
        for (size_t i = 0; i < memberCount; i++)
        {
            int memberId;
            int adderId;
            members = deserialize(members, (memberCount - i) * 8, "ii", &memberId, &adderId);
