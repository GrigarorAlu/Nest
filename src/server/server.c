                ...
                if (userId)
                {
                        ...
                        printf("\tType: GET_MESSAGES.\n");
                        {
                            ...
                            while (sqlite3_step(stmt) == SQLITE_ROW)
                            {
                                ...
                                memcpy(serializeInto(packet->data, "hiiiht", COMMAND_GET_MESSAGES,
                                    messageId, chatId, senderId, type, sent), content, contentLen);
                    ...
                    case COMMAND_CHAT_ADD:
                        printf("\tType: CHAT_ADD.\n");
                        {
                            int chatId;
                            char *username = 0;
                            if (deserialize(data, dataLen, "is", &chatId, &username) != data + dataLen)
                            {
                                printf("\tInvalid format.\n");
                                free(username);
                                break;
