...
static void AddCodepointRange(Font *font, const char *path, int start, int stop)
{
    ...
    UnloadFont(*font);
    *font = LoadFontEx(path, 200, updatedCodepoints, updatedCodepointCount);
...
void UI_open()
{
    ...
    username = strdup("");
    password = strdup("");
    font = LoadFontEx("res/UbuntuRegular.ttf", 200, 0, 0);
    AddCodepointRange(&font, "res/UbuntuRegular.ttf", 0x80, 0x17f);
    AddCodepointRange(&font, "res/UbuntuRegular.ttf", 0x400, 0x45F);
    AddCodepointRange(&font, "res/UbuntuRegular.ttf", 0x25A0, 0x25CF);
    ...
    else
    {
        ...
        case MENU_CHAT_MESSAGE:
            ...
            for (size_t i = 0; i < messageCount; i++)
            {
                ...
                if (message->senderId != accountId && (i == messageCount - 1 ? 1 :
                        message->senderId != messages[i + 1].senderId))
                {
                    bool userLoaded = false;
                    for (size_t j = 0; j < userCount; j++)
                        if (users[j].id == message->senderId)
                        {
                            if (users[j].name)
                    ...
                    if (!userLoaded)
                    {
                        addUser((User){ message->senderId, 0, 0, 0 });
                        userRequest(message->senderId);
                    }
                    posY += nameSize;
                }
            }
...
void addMessage(Message message)
{
    ...
    for (size_t i = 0; i < messageCount; i++)
        if (message.id == messages[i].id)
            return;
        else if (message.id > messages[i].id)
        {
